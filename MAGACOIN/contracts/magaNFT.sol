...
// @dev This interface defines the functions for interacting with the MagaCoin contract.
interface  MagaCoin {
    function mint(address userAddress, uint256 amount)external;   
}

contract MagaNFT is ERC721,Ownable {
    ...
    function random() private returns (uint) {
        uint minMonths = 7;
        uint maxMonths = 11;
        uint range = (maxMonths - minMonths) + 1;
        uint months = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, block.number))) % range;
        months = months + minMonths;
        return months;
    }

    /** 
     * @dev Updates the price of the NFT.
     * @param _price The new price for the NFT.
     */
    function updateNFTPrice(uint256 _price)external onlyOwner{
        nftPrice = _price;
        emit UpdateNFTPrice(nftPrice);
    }
