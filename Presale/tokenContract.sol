...
contract TestCoin is ERC20,Ownable {    
     ...
     */
   constructor(address _trasuryWallet) ERC20("TestCoin", "Test") Ownable(msg.sender){
        uint256  totalSupply= 200* 10**9 *(10**decimals());
        trasuryWallet=_trasuryWallet;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if(isBuy || isSell){
            Tax = amount * (TAX) / (100);
            walletTax= Tax/2;
            burnTax = Tax - walletTax;
            _update(sender, trasuryWallet, walletTax);
            _burn(sender,burnTax);
            Tax= walletTax + burnTax;
            amount -= Tax;
        }
    ...
    /**
     * @dev Distributes tokens to multiple addresses in a single function call.
     * @param addresses An array of recipient addresses.
     * @param tokens An array of token amounts corresponding to each recipient address.
     */
    function airdrop(address[] calldata addresses, uint[] calldata tokens) external onlyOwner {
        ...
        for(uint i=0; i < addresses.length; i++){
            uint amount = tokens[i];
            _update(msg.sender,addresses[i],amount);
        }
    }
}