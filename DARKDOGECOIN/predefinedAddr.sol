...
contract Create {
    ...
    function deployContract(bytes32 _salt) external returns (bool) {
        ...
        addr = Create2.deploy(
            0,
            _salt,
            abi.encodePacked(type(DARKDOGECOIN).creationCode, abi.encode(charityWallet, marketingWallet))
        );
    ...
    // perfect working with two digit like:- 0x24
     function findAddressWithPrefix(uint256 start,uint256 end,bytes memory startCode) public view returns (bytes32, address) {
        ...
        for (uint256 i = start; i < end; i++) {
            ...
            salt,
            keccak256(abi.encodePacked(type(DARKDOGECOIN).creationCode, abi.encode(charityWallet, marketingWallet)))
        );
