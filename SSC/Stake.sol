...
contract Stake is Ownable {
    ...
    uint256 private onemonth = 31 days;
     ...
     *
     */
    function rewardInPercentage() public view returns (uint256) {
     ...
     *
     */
    function totalProfitedAmt(address user, uint256 _stakeid)
        public
        view
        returns (uint256)
    {
    ...
    function stake(uint256 _stakeamount) public returns (bool) {
        ...
        stakebalance =
            _stakeamount +
            (_stakeamount * (bonusPercentage / 1000)) + profit;//5.24983e18 = stake token+ 50% of reward(8%) + profit(8% of stake amount)
        rewardPoolNewBal = rewardPoolOldBal - stakebalance;
        rewardInPercentage();// for balance update
    ...
    function unStake(uint256 _stakeid) public returns (bool){        
        ...
        if(block.timestamp > locktime){
            ...
            rewardPoolOldBal=rewardPoolNewBal;
            profit= (staking[user][_stakeid]._profit*25)/100;
            uint penalty = viewPenalty(_stakeid);
    ...
    function viewWithdrawAmount(uint256 _stakeid) public view returns (uint256){        
        ...
        if(block.timestamp > locktime){
            ...
            profit= (staking[user][_stakeid]._profit*25)/100;
            uint totstakeAmt=staking[user][_stakeid]._amount-viewPenalty(_stakeid);         
            totalAmt= totstakeAmt+profit;
        }else if(block.timestamp > twoWeekLocktime){           
            profit= (staking[user][_stakeid]._profit*35)/100;
            uint totstakeAmt=staking[user][_stakeid]._amount-viewPenalty(_stakeid); 
