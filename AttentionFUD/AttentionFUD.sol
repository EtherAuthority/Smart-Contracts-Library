...
contract AttentionFUD is AdminControl, ERC1155Receiver{
    ...
    */
    ...
    function depositTokens( uint256 total,string memory project ) external {
        ...
        require(bytes(project).length > 0 , "AttentionFUD : Bad project name");
    ...
    /**
     * Unstake tokens from a project
     */
    function withdrawTokens(string memory project) external {
        require(stakeIsActive, "AttentionFUD: staking must be active to withdraw Tokens!");
        ...
        require(amount>0,"AttentionFUD: You have not staked amount on this project");
     ...
     */
    function burnDeposit() external adminRequired{
        uint256[] memory projectBurn=deleteAllProjectsAndDeposits();
        uint256 total = projectBurn[1];
        ERC1155Burnable(contractAddressFUD).burn(address(this),7123, total); 
        emit BurnDeposit(projectBurn[1],projectBurn[0]);
    }
    ...
    /**
     * @notice Deletes all projects and their associated deposits.
     * @dev This function iterates through all projects, deletes all associated deposits,
     *      resets the necessary mappings, and returns the total number of projects deleted
     *      and the total amount of tokens burned.
     * @return A uint256 array where the first element is the number of projects deleted
     *         and the second element is the total amount of tokens burned.
     */
    function deleteAllProjectsAndDeposits() private returns (uint256[] memory) {
        uint256 burnedProjectsCount = 0;
        uint256 totalTokensBurned = 0;

        for (uint256 i = 0; i < projects.length; i++) {
            string memory projectName = projects[i].project;

            // Delete all deposits for the project
            address[] memory wallets = stakesByProject[projectName].wallet;
            for (uint256 j = 0; j < wallets.length; j++) {
                address wallet = wallets[j];
                totalTokensBurned += stakesByProject[projectName].amount[wallet];
                // Manually reset the nested mapping entry to 0
                stakesByProject[projectName].amount[wallet] = 0;
                delete stakesByAddress[wallet]; // Delete stakesByAddress entry
            }

            // Delete the project itself
            delete stakesByProject[projectName].wallet;
            stakesByProject[projectName].inUse = false;
            stakesByProject[projectName].index = 0;
        ...
        // Return the number of burned projects and total tokens burned
        uint256[] memory burnedInfo = new uint256[](2);
        burnedInfo[0] = burnedProjectsCount;
     ...
     *         - A string array of project names.
     *         - A uint256 array of token amounts staked by the given wallet in each project.
     */
    function getTokensStake(address wallet) external view returns (string[] memory, uint256[] memory) {
        uint256 numProjects = projects.length;
        string[] memory projectNames = new string[](numProjects);
        uint256[] memory tokenAmounts = new uint256[](numProjects);
    
        for (uint256 i = 0; i < numProjects; i++) {
            string memory projectName = projects[i].project;
            uint256 tokensStakedInProject = stakesByProject[projectName].amount[wallet];
    
            projectNames[i] = projectName;
            tokenAmounts[i] = tokensStakedInProject;
        }

        return (projectNames, tokenAmounts);
    }

    /**
     * @notice Retrieves details of all stakes across all projects.
     * @dev This function iterates through all projects and their associated wallets to compile
     *      a list of unique stakes, avoiding duplicates, and returns an array of StakeDetail structs.
     * @return An array of StakeDetail structs containing project names, staked amounts, and stakeholder addresses.
     */
    function getStakes() external view returns (StakeDetail[] memory) {
        // Calculate the total number of stakes
        uint256 totalStakeCount = 0;
        for (uint i = 0; i < projects.length; i++) {
            totalStakeCount += stakesByProject[projects[i].project].wallet.length;
        }
    
        // Prepare the array to hold all the stake details
        StakeDetail[] memory stakeDetails = new StakeDetail[](totalStakeCount);
    
        uint256 currentIndex = 0;
        for (uint i = 0; i < projects.length; i++) {
            string memory projectName = projects[i].project;
            for (uint j = 0; j < stakesByProject[projectName].wallet.length; j++) {
                address stakeholder = stakesByProject[projectName].wallet[j];
                uint256 amount = stakesByProject[projectName].amount[stakeholder];
    
                if (amount > 0) {
                    bool isDuplicate = false;
                    // Check for duplicate entries
                    for (uint k = 0; k < currentIndex; k++) {
                        if (stakeDetails[k].stakeholder == stakeholder && keccak256(abi.encodePacked(stakeDetails[k].project)) == keccak256(abi.encodePacked(projectName))) {
                            isDuplicate = true;
                            break;
                        }
                    }
                    if (!isDuplicate) {
                        stakeDetails[currentIndex] = StakeDetail({
                            project: projectName,
                            amount: amount,
        ...
        // Create a trimmed array to exclude unused slots
        StakeDetail[] memory trimmedStakeDetails = new StakeDetail[](currentIndex);
        for (uint i = 0; i < currentIndex; i++) {
            trimmedStakeDetails[i] = stakeDetails[i];
        }
    
        return trimmedStakeDetails;
    }
    
    /**
     * @notice Calculates the total amount of tokens staked on a given project.
     * @dev This function sums up all tokens staked by various wallets in the specified project.
     * @param projectName The name of the project for which the total staked tokens are to be calculated.
     * @return The total amount of tokens staked in the specified project.
     */
    function getTotalTokensStakedOnProject(string memory projectName) private view returns (uint256) {
        require(stakesByProject[projectName].inUse, "AttentionFUD: Project not found.");
    
        uint256 totalTokensStaked = 0;
        address[] memory wallets = stakesByProject[projectName].wallet;
    
        for (uint256 i = 0; i < wallets.length; i++) {
            totalTokensStaked += stakesByProject[projectName].amount[wallets[i]];
        }
    ...
    /**
     * Methods needed to be an ERC1155 Batch Reciever
     */
    function onERC1155BatchReceived(
        address,
        address,
