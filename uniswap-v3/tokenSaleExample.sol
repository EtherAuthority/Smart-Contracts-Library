...
contract TokenSale is Ownable{
    ...
    uint256 public rate; // Price of the token
    ...
    // Token Buy
    function buyTokens(address referrer, uint256 tokenAmount) external payable {

        require(block.timestamp >= startTime, "Token sale not started yet");
        ...
        if (referrer != address(0)) {
            uint256 bonus = (tokenAmount * referrerRewardPercent) / 100; // Calculating the referral bonus 
    ...
    */
    ...
    function withdrawLeftoverTokens() external onlyOwner{
        require(block.timestamp > endTime, "Token sale has not ended yet");
