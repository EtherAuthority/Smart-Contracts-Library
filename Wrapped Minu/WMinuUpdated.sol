...
contract WrappedMinu is ERC20, Ownable {
    ...
    function wrap(uint256 amount) external {
        ...
        uint256 burnAmount = _calculateTax(amount, burnPercentOnSwap);
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        require(amount > 0, "Amount be more than 0");
        require(balanceOf(msg.sender) >= amount, "Not sufficient balance");

        uint256 burnAmount = 0;
        if (!_isExcludedFromFee[msg.sender]) {
            burnAmount = _calculateTax(amount, burnPercentOnTransfer);
            amount -= burnAmount;
    ...
    function setBurnPercentOnSwap(uint256 percent) external onlyOwner {
        require(percent <= 5000 && percent > 0, "Burn percent cannot be more than 5%");
        burnPercentOnSwap = percent;
    }

    function setBurnPercentOnTransfer(uint256 percent) external onlyOwner {
        require(percent <= 5000 && percent > 0, "Burn percent cannot be more than 5%");
        burnPercentOnTransfer = percent;
    }
